  ...
  return (
    <header className="bg-slate-800 text-white shadow-lg rounded-xl mb-6 flex flex-col sm:flex-row items-center justify-between px-6 py-4">
      ...
      <nav className="flex items-center gap-6 mt-2 sm:mt-0">
        ...
        <Link href="/cart" className="hover:text-yellow-400 transition-colors font-medium flex items-center gap-1">
          <span role="img" aria-label="cart">🛒</span> Cart <span className="bg-orange-400 text-white rounded-full px-2 py-0.5 text-xs font-bold ml-1">{isHydrated ? totalItemCount : 0}</span>
        </Link>
      </nav>
    </header>
  );
