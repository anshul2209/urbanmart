    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 items-start gap-4 sm:gap-6 lg:gap-10">
        <div className="relative w-full max-w-sm mx-auto md:mx-0 md:max-w-xl lg:max-w-2xl aspect-[3/2] md:aspect-square bg-gray-50 rounded-lg overflow-hidden md:justify-self-start">
          <Image src={mainImage} alt={product.title} fill sizes="(max-width: 640px) 90vw, (max-width: 1024px) 50vw, 600px" className="object-cover" priority />
        </div>
        <div className="flex flex-col gap-3 sm:gap-4 md:pl-4 lg:pl-6">
          <h2 className="m-0 text-lg sm:text-xl md:text-2xl font-semibold break-words">{product.title}</h2>
          <p className="m-0 text-gray-700 text-sm md:text-base break-words leading-relaxed">{product.description}</p>
          <div className="flex flex-wrap gap-3 items-center">
            <strong className="text-base md:text-lg">${product.price.toFixed(2)}</strong>
            <span className="text-green-500 text-sm md:text-base">-{product.discountPercentage}%</span>
